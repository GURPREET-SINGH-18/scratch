...
function PreviewArea({ character, add_character, set_active }) {
  ...
  const handleChange = (e) => {
    ...
    enqueueSnackbar((e.target.value)+" is active", { variant: "info" });
  };
        ...
        <div>
          <Button
            ...
            onClick={() => {add_character()
              enqueueSnackbar("created sprite-"+(character.characters.length-1), { variant: "success" });
            }}
          >
            Create{" "}
          </Button>
          ...
          return (
            ...
            >
              <div id={`${x.id}-div`} className="character">
                <div
                  className="hidden border-2 p-2 ml-3 mb-2 w-auto whitespace-nowrap rounded-full"
                  id={x.id + "-message-box"}
                ></div>
                <div
